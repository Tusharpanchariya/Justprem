...
function createInvoicePdf(details) {
    ...
    const commands = ["0.055 0.106 0.063 rg 0 744 595 98 re f", "0.94 0.91 0.85 rg 42 704 511 1 re f"];
    const text = (value, x, y, size = 10, bold = false, color = "0.12 0.11 0.09") => commands.push(`${color} rg BT /${bold ? "F2" : "F1"} ${size} Tf 1 0 0 1 ${x} ${y} Tm (${escapePdfText(value)}) Tj ET`);
    const rule = (y, x = 42, width = 511, color = "0.86 0.83 0.77") => commands.push(`${color} RG ${x} ${y} m ${x + width} ${y} l S`);
    ...
    let y = 520;
    rule(y + 18);
    text("ITEM", 42, y, 9, true, "0.36 0.25 0.15");
    text("QTY", 385, y, 9, true, "0.36 0.25 0.15");
    text("AMOUNT", 475, y, 9, true, "0.36 0.25 0.15");
    rule(y - 8);
    y -= 28;
    for (const item of details.items.slice(0, 8)) {
        ...
        text(String(item.quantity), 390, y, 10);
        text(`EUR ${(item.priceEUR * item.quantity).toFixed(2)}`, 469, y, 10);
        y -= Math.max(28, itemLines.slice(0, 2).length * 13 + 15);
        rule(y + 8);
    }
    if (details.discount) {
        y -= 4;
    ...
    y -= 12;
    text(paymentPending ? "TOTAL DUE" : "TOTAL PAID", 363, y, 11, true);
    text(`EUR ${details.cartTotal.toFixed(2)}`, 457, y, 13, true, "0.36 0.25 0.15");
    rule(y - 13, 350, 203, "0.36 0.25 0.15");
