...
export interface OrderDetails { email: string; phone: string; name: string; address: string; city: string; region: string; country: string; postalCode: string; items: OrderItem[]; cartTotal: number; paymentId: string; paymentMethod: string; couponCode?: string; discount?: number; deliveryFee?: number }
...
function createCustomerEmail(details: OrderDetails) {
  ...
  const deliveryRow = details.deliveryFee ? `<tr><td style="padding-top:12px;color:#0e1b10;font-size:14px">Delivery</td><td style="padding-top:12px;text-align:right;color:#0e1b10;font-size:14px">${money(details.deliveryFee)}</td></tr>` : "";
