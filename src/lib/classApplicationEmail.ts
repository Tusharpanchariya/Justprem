...
type ClassApplication = { name: string; email: string; phone: string; courseName: string; bookingDate: string };
...
export async function sendClassApplicationEmail(application: ClassApplication) {
  ...
  const bannerPath = path.join(process.cwd(), "assests", "thanksforclassinroll.png");
  ...
  const attachments = [{ filename: "thanksforclassinroll.png", content: banner, contentType: "image/png", contentId: "justprem-harmonium" }];
